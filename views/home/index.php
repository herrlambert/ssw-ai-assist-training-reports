<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>SSW Training Reports</title>
    <link rel="stylesheet" href="/mattlam/ssw-ai-assist-training-reports/assets/css/style.css">
</head>
<body>
    <header class="banner">
        <a href="https://www.washington.edu" class="banner-logo">
            <img src="https://www.washington.edu/brand/files/2014/09/W-Logo_Purple_RGB.png" alt="W Logo">
            <span class="uw-text">University of Washington</span>
        </a>
        <h1>SSW Training Reports</h1>
    </header>
    
    <main class="container">
        <nav class="reports-menu">
            <h2>Available Reports</h2>
            <ul>
                <li>
                    <a href="/mattlam/ssw-ai-assist-training-reports/reports/by-person" class="report-link">
                        Find Trainings by Person
                    </a>
                </li>
                <li>
                    <a href="/mattlam/ssw-ai-assist-training-reports/reports/expired" class="report-link">
                        Show Expired Trainings
                    </a>
                </li>
            </ul>
        </nav>
    </main>
</body>
</html>



